...
def fetch_users(client_id,site_id):
    ...
    while True:
        ...
        js=r.json()
        users=[]
        if isinstance(js,list): users=js
        elif isinstance(js,dict): users=js.get("users") or js.get("data") or js.get("items") or []
        if not users: break
        for u in users:
...
def send_message(room_id,text):
    ...
    except Exception as e: log.error(f"Webex send: {e}")

def send_adaptive_card(room_id):
    payload={"roomId":room_id,"text":"✍ Vul dit formulier in:",
...
def create_halo_ticket(form,room_id):
    ...
    if not r.ok:
        ...
        log.error(r.text); return
    t=r.json()
    tid=str(t.get("TicketNumber") or t.get("id") or t.get("TicketID"))
    USER_TICKET_MAP.setdefault(room_id,[]).append(tid)
    TICKET_STATUS_TRACKER[tid]={"status":t.get("StatusName") or t.get("status","Unknown"),"last_note":0}
...
def check_ticket_status_and_notes():
    ...
    for tid,track in list(TICKET_STATUS_TRACKER.items()):
        try:
            ...
            if t.ok:
                js=t.json(); cur=js.get("StatusName") or js.get("status","Unknown")
                if cur!=track["status"]:
                    track["status"]=cur
                    for rid,ids in USER_TICKET_MAP.items():
                        if tid in ids: send_message(rid,f"⚠️ Ticket #{tid} status → {cur}")
            ...
            if a.ok:
                ...
                if acts:
                    latest=acts[-1]; note=latest.get("outcome") or latest.get("note") or ""
                    if note and note!=track.get("last_note_text"):
                        track["last_note_text"]=note
                        for rid,ids in USER_TICKET_MAP.items():
                            if tid in ids: send_message(rid,f"🗒 Nieuwe note in Halo #{tid}\n{note}")
        except Exception as e: log.error(f"check {tid}: {e}")
...
def process_webex_event(payload):
    ...
    text=msg.get("text",""); room_id=msg.get("roomId"); sender=msg.get("personEmail","")
    if sender.endswith("@webex.bot"): return
    if "nieuwe melding" in text.lower():
        send_message(room_id,"👋 Hi! Klik hieronder om een ticket te maken:")
        send_adaptive_card(room_id); return
    ticket=re.search(r'Ticket #(\d+)',text)
    if ticket:
        tid=ticket.group(1)
        if any(tid in t for t in USER_TICKET_MAP.values()):
            add_public_note(tid,text); send_message(room_id,f"📝 Note toegevoegd aan #{tid}.")
        else: send_message(room_id,f"❌ Ticket #{tid} niet gevonden."); return
