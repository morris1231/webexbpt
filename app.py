...
# ⚠️ echt de ID van "Public Note" uit Admin > Configuration > Action Types
...
def get_halo_headers():
    ...
    r = requests.post(HALO_AUTH_URL, headers={
...
def get_halo_user_by_email(email):
    ...
    print("🔎 User lookup", r.status_code, r.text)
...
def create_halo_ticket(summary, naam, email, omschrijving="", sindswanneer="", watwerktniet="",
    ...
    if ticket_id:
        ...
        # 🔥 Altijd een Public Note met vragenlijst zetten
        note_payload = [{
            "TicketID": ticket_id,
        ...
        print("📝 Public Note resp:", nr.status_code, nr.text[:200])
...
def add_note_to_ticket(ticket_id, text, sender="Webex"):
    ...
    print("💬 Add note resp:", r.status_code, r.text[:200])
...
def webex_webhook():
    ...
    if resource == "messages":
        ...
        if "nieuwe melding" in text.lower():
            send_message(room_id, "✍ Vul de kaart in om een melding aan te maken.")
        else:
        ...
        ticket = create_halo_ticket(summary, naam, email,
            omschrijving, sindswanneer, watwerktniet, zelfgeprobeerd,
...
# Halo webhook voor Public Notes → terug naar Webex
