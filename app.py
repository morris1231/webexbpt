...
def get_halo_user_by_email(email):
    ...
    if r.status_code == 200 and isinstance(r.json(), list) and len(r.json()) > 0:
        user = r.json()[0]
        return user.get("ID"), user.get("CustomerID")  # CustomerID = None → agent
    return None, None
...
def webex_webhook():
    ...
    if resource == "messages":
        ...
        if "nieuwe melding" in text.lower():
            # stuur adaptive card
            send_message(room_id, "Stuur formulier via adaptive card (nog toevoegen code)")
        else:
