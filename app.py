...
# ✅ Zet deze goed in je .env
HALO_ACTIONTYPE_PUBLIC = int(os.getenv("HALO_ACTIONTYPE_PUBLIC", "0"))
...
def get_halo_user_by_email(email):
    ...
    h = get_halo_headers()
    resp = requests.get(f"{HALO_API_BASE}/Users?$filter=Email eq '{email}'", headers=h)
