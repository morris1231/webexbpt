...
HALO_TICKET_TYPE_ID = int(os.getenv("HALO_TICKET_TYPE_ID", 66))  # ✅ 66 (verplichte tickettype ID)
HALO_TEAM_ID        = int(os.getenv("HALO_TEAM_ID", 35))         # ✅ Team ID (controleer in Halo)
HALO_CLIENT_ID_NUM  = int(os.getenv("HALO_CLIENT_ID_NUM", 12))   # ✅ Client ID (controleer in Halo)
HALO_SITE_ID        = int(os.getenv("HALO_SITE_ID", 18))         # ✅ Site ID (controleer in Halo)
...
CACHE_DURATION = 24 * 60 * 60  # ✅ CORRECTE MULTIPLICATIE (24 uur)
ticket_room_map = {}
...
# --------------------------------------------------------------------------
# USERS OPHALEN — MET PAGINATIE — ALLE 45+ ECHTE USERS
# --------------------------------------------------------------------------
...
# --------------------------------------------------------------------------
# TICKET CREATION — ZONDER GENERAL USER — MET JUISTE TEAM EN TICKETTYPE
# --------------------------------------------------------------------------
def create_halo_ticket(omschrijving, email, sindswanneer, watwerktniet,
    ...
    # ✅ CRUCIAAL: Gebruik DE JUISTE VELDNAMEN VOOR HALO v22.08
    body = {
        "summary": omschrijving[:100],
        "details": omschrijving,
        "Type": HALO_TICKET_TYPE_ID,  # ✅ HOOFDLETTER T + INTEGER (VERPPLICHT)
        "teamId": HALO_TEAM_ID,
        "impact": int(impact_id),
        "urgency": int(urgency_id),
        "requestedbyid": user_id,    # ✅ CORRECTE USER ID VELD (geen requestUserId)
        "client_id": client_id,
        "site_id": site_id,
    }
    ...
    try:
        ...
        log.info(f"⬅️ Halo antwoord {r.status_code}: {r.text[:250]}")
        if r.status_code in (200, 201):
            resp = r.json()
            ticket = resp[0] if isinstance(resp, list) else resp
            ticket_id = ticket.get("id") or ticket.get("ID")
            msg = f"✅ Ticket aangemaakt → TicketID={ticket_id} | RequestedBy: {user.get('name')} | Team: {HALO_TEAM_ID}"
            log.info(msg)
            if room_id: send_message(room_id, msg)
            return {"ID": ticket_id, "user_id": user_id}
    except Exception as e:
        log.error(f"❌ Request faalde: {e}")
    
    if room_id:
        send_message(room_id, "❌ Ticket aanmaken mislukt. Controleer de logs.")
    return None
...
# START
# --------------------------------------------------------------------------
if __name__ == "__main__":  # ✅ CORRECTE __name__ SYNTAX
