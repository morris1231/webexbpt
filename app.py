...
# USERS
# --------------------------------------------------------------------------
def fetch_users(c_id:int, s_id:int):
    h = get_halo_headers()
    res=[]
    r = requests.get(f"{HALO_API_BASE}/Users",
                     headers=h,
                     params={"client_id":c_id,"site_id":s_id},
                     timeout=20)
    if r.ok:
        users=(r.json().get("users") or
               ...
               r.json())
        for u in users:
            if not u.get("inactive") and "@" in u.get("emailaddress",""):
                res.append(u)
    return res
...
def get_user(email):
    ...
    for u in get_users():
        for k in ("EmailAddress","emailaddress","PrimaryEmail","login","email","email1"):
            v=u.get(k)
...
def create_halo_ticket(form, room_id):
    ...
    if not user:
        send_message(room_id,"❌ Geen gebruiker gevonden in Halo.");return
    user_id=int(user["id"])
    body={
    ...
    if not r.ok:
        send_message(room_id,f"⚠️ Ticket aanmaken mislukt: {r.status_code}");return
    tid=r.json()[0]["id"]
    TICKET_ROOM_MAP[room_id]=tid
    # voeg lijst-inhoud als eerste public note toe
    note=("\n".join([
...
def add_public_note(ticket_id,text):
    h=get_halo_headers()
    requests.post(f"{HALO_API_BASE}/Tickets/{ticket_id}/Notes",
                  headers=h,json={"text":text,"is_public":True},timeout=15)

def check_new_halo_notes():
    """polling alternatief voor Halo webhook: haalt nieuwe public notes op en stuurt ze naar Webex"""
    while True:
        try:
            h=get_halo_headers()
            for room_id,ticket_id in list(TICKET_ROOM_MAP.items()):
                r=requests.get(f"{HALO_API_BASE}/Tickets/{ticket_id}/Notes",
                               headers=h,timeout=15)
                if not r.ok: continue
                notes=r.json()
                latest=max((n["id"] for n in notes if n.get("is_public")),default=0)
                last=TICKET_ROOM_MAP[ticket_id] if isinstance(TICKET_ROOM_MAP.get(ticket_id),dict) else None
                if not last or latest>last.get("last_note",0):
                    new=[n for n in notes if n.get("is_public") and n["id"]>last.get("last_note",0) if last else True]
                    for n in new:
                        send_message(room_id,f"📢 **Public note in Halo:**\n{n['text']}")
                    TICKET_ROOM_MAP[room_id]={"ticket_id":ticket_id,"last_note":latest}
        except Exception as e:
...
def process_webex_event(payload):
    ...
    if res=="messages":
        mid=payload["data"]["id"]
        msg=requests.get(f"https://webexapis.com/v1/messages/{mid}",
...
def halo_hook():
    ...
    if not note or not ticket_id: return {"status":"ignore"}
...
def health():
    return {"status":"running","tracked_rooms":len(TICKET_ROOM_MAP)}
