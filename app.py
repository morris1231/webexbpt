...
# Zet in je .env de juiste ID van PUBLIC NOTE action type uit Halo Admin
...
def create_halo_ticket(summary, naam, email,
    ...
    ticket = {
        "Summary": summary,
        "Description": description,  # → Request Details veld
    ...
    if customer_id: ticket["CustomerID"] = customer_id
    if user_id: ticket["CustomerUserID"] = user_id  # koppeling end-user
    ...
    if ticket_id:
        ...
        # Altijd public note met de vragenlijst → zichtbaar in Progress Feed
        note_payload = [{
            "TicketID": ticket_id,
...
def webex_webhook():
    ...
    if resource == "messages":
        ...
        if "nieuwe melding" in text.lower():
            send_adaptive_card(room_id)  # Stuur de kaart!
        else:
